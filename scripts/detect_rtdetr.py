"""RT-DETR-v2 漫画文本检测器（ONNX，CPU 友好）。

从 BallonsTranslator detector_ctbd.py 精简移植：
- ONNX 推理（640x640 输入，ogkalu/comic-text-and-bubble-detector）
- 高瘦图自动切片（ImageSlicer）
- 后处理：重复框合并 + 包含框移除
- 输出扁平 blocks[]（bbox + source_engine），与 amta detect_station 输出对齐

用法:
    from detect_rtdetr import RTDetrDetector
    det = RTDetrDetector()
    blocks = det.detect(image_path)  # -> [{"bbox": [x1,y1,x2,y2], "source_engines": ["rtdetr-v2"], ...}]
"""
...
class ImageSlicer:
    """高瘦图（H/W > threshold）自动切片检测，切片间重叠，结果合并。"""
...
class RTDetrDetector:
    ...
    def _detect_single(self, image: np.ndarray) -> np.ndarray:
        ...
        # 预处理：resize 640x640, BGR→RGB, 归一化
    ...
    def detect(self, image: str | Path | np.ndarray,
        ...
        """检测图像中的文本框，返回 amta 兼容的 blocks[]。

        Args:
            image: 图像路径或 BGR numpy 数组
            conf_threshold: 可选覆盖置信度阈值

        Returns:
            [{"bbox": [x1,y1,x2,y2], "source_engines": ["rtdetr-v2"],
              "bubble_type": "text_free"|"text_bubble", "confidence": float}]
        """
        if conf_threshold is not None:
            self.conf_threshold = conf_threshold
        if isinstance(image, (str, Path)):
            # cv2.imread 不支持中文路径，用 fromfile + imdecode
...
if __name__ == "__main__":
    import sys
    det = RTDetrDetector()
    img_path = sys.argv[1] if len(sys.argv) > 1 else r"D:\我的汉化\汉化作品\东方\单翼停留之地\11.jpg"
    blocks = det.detect(img_path)
    print(f"Detected {len(blocks)} text boxes in {det._last_time:.2f}s")
    for b in blocks:
        print(f"  {b['region_id']}: {b['bbox']}")
