...
## 十二、选项 B 最终结果：lama-manga 本地推理（2026-09-04 执行后更新）

### 12.1 实现概要

在分支 `feat/stage4-lama-manga-local-inference` 上完成：
1. 从 BallonsTranslator 复制 FFC ResNet Generator 模型定义（`src/amta/_lama_ffc.py`），989 个权重键 100% 匹配
2. `LocalLamaInpainter` 新增 `model_type="lama-manga"` 支持
3. 实验脚本新增 `p1_manga` 模式，A/B 报告新增第 4 对比组

### 12.2 关键发现一：预处理/后处理必须对齐 Koharu

最初本地推理出现严重黑/白噪点（模型输出范围 [-307, 8755]），根因是两个预处理错误：

| 环节 | 我们（错误） | Koharu（正确） |
|---|---|---|
| 输入归一化 | `[-1, 1]`（`/127.5 - 1`） | `[0, 1]`（`/255.0`） |
| 输出激活 | 无（`add_out_act=False`） | **sigmoid**（限制到 [0,1]） |

修复后（commit `e9956b6`）：输出范围正常 [0,1]，噪点完全消除。

### 12.3 关键发现二：整页推理 > 裁剪推理（质量）

裁剪推理（padding=64）时，模型只有局部上下文，会把背景猜成纯白，形成明显白色方框。

**整页推理**（缩小到 1024 宽，整页送模型，放大回原尺寸）时，模型有全局上下文，背景修复和周围完美融合，无方框。

| 方案 | 速度 | 质量 |
|---|---|---|
| 裁剪推理（p1_manga） | 7.3s/页 | 有白色方框 ❌ |
| **整页推理（p1_manga）** | **19.7s/页** | **接近 Baseline，无方框** ✅ |

整页推理比 Baseline（~152s/页）快 **87%**，质量基本持平。

### 12.4 最终速度数据（5页×1次，整页推理）

| 页 | free框 | Baseline（整页Koharu） | p1_manga（整页本地推理） |
|---|---|---|---|
| 11 | 3 | 111.7s | 17.3s |
| 12 | 1 | 38.0s | 18.6s |
| 13 | 3 | 275.3s | 20.1s |
...
- ✅ 白色方框消除（整页推理）
- ✅ 网点背景完美贴合，颜色一致
- ✅ 框外字去除（detection 覆盖范围内）
- ⚠️ detection 漏检：部分竖排文字/拟声词未被检测到（如 page11"はっ……"、page13"ビッ"），Baseline 也有同样问题，属于 detection 模块而非 inpainting 问题

### 12.6 测试覆盖

- `tests/test_lama_ffc.py`：4/4 通过（模型定义、权重加载、前向传播）
- `tests/test_local_lama_inpainter_manga.py`：4/4 通过（模型加载、输出形状、非正方形、默认类型）
- `tests/test_exp_p1_manga.py`：2/2 通过（模式注册、单页运行）

### 12.7 commit 记录（选项 B 分支）

- `d39bc76` FFC ResNet 模型定义（从 BallonsTranslator 复制）
- `5992466` LocalLamaInpainter 支持 lama-manga
- `debce0b` 修复 build_rect_mask h/w swap bug
- `677233c` 灰度化输出 + 羽化 mask（中间方案，后续被替代）
- `e9956b6` 对齐 Koharu 预处理/后处理（输入 [0,1] + 输出 sigmoid）
- `9b2513d` 整页推理模式（最终方案，消除白色方框）

### 12.8 最终决策

**推荐方案：p1_manga 整页本地推理**

- 速度：19.7s/页（比 Baseline 快 87%）
- 质量：接近 Baseline（Koharu），无噪点、无方框、网点贴合
- 模型：lama-manga（漫画微调模型，已在本地，无需下载）
- 依赖：PyTorch CPU（无需 GPU、无需 Koharu HTTP 服务）

**后续优化方向（非阻塞）**：
1. detection 模块优化，减少竖排文字/拟声词漏检
2. 探索 GPU/DirectML 加速，进一步降低推理时间
3. 多尺度推理，提升大尺寸图片的细节质量
