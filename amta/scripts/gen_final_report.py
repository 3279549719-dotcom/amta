"""生成三阶段流水线最终 HTML 报告（自包含，内嵌原图 base64）。

用法: python scripts/gen_final_report.py --work-id integrate-final --src-dir <原图目录> --out <output.html>
"""
...
def gen_report(work_id: str, src_dir: Path, out_path: Path, pages: list[int]) -> None:
...
"""]
    ...
    print(f"Report generated: {out_path} ({out_path.stat().st_size // 1024} KB)")
...
def main() -> int:
    ...
    a = ap.parse_args()
    gen_report(a.work_id, a.src_dir, a.out, a.pages)
    return 0
