"""memory_inject — DSH 原生记忆注入：把记忆包写进 CLAUDE.local.md（随会话自动加载）。
...
用法：
  python scripts/memory_inject.py             # 生成/刷新 amta/CLAUDE.local.md（默认）
  python scripts/memory_inject.py --stdout    # 只打印记忆包到 stdout（调试 / 外部消费）
  python scripts/memory_inject.py --budget N  # 自定义包预算（默认 4096）
  python scripts/memory_inject.py --source X  # startup|compact|resume|clear|fork（默认 startup）
  python scripts/memory_inject.py --target P  # 覆盖输出路径（默认 <root>/CLAUDE.local.md）
...
DEFAULT_BUDGET = 4096  # DSH 本地 overlay 包预算（CC hook 1.5KB 上限不适用于文件加载；4KB 兼顾内容与成本）
SOURCES = ("startup", "compact", "resume", "clear", "fork")

# 仓库根 = 本脚本的上一级（amta）。AGENTS.md/CLAUDE.md 与记忆地产 .remember/ 都在此，
...
def _pack(root: Path, source: str, budget: int) -> str:
    ...
    except Exception:  # noqa: BLE001 — 注入纪律：地产异常降级，不阻塞会话/生成
        return "=== AMTA 记忆包 (degraded) ===\n记忆地产读取异常：先跑 python scripts/memory_status.py"
