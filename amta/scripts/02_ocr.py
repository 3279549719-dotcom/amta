...
引擎可插拔: baberu(默认,ONNX内置) / hayai(HayaiOCR-v2.1) / manga_ocr(kha-white)。
...
def run(work_id: str, det_path: Path, raw_page: Path, out_path: Path,
        page_idx: int = 0, engine: str = "baberu", vlm_enabled: bool = False,
...
def main() -> int:
    ...
    ap.add_argument("--engine", default="baberu", choices=["baberu", "hayai", "manga_ocr"],
                    help="OCR 引擎（默认 baberu，ONNX 内置免费）")
