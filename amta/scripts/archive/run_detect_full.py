"""全量 42 页 RT-DETR-v2 检测 + Baberu OCR，输出快速统计报告。

用法: python scripts/run_detect_full.py
输出: output/data/detect_full/results.json + 控制台汇总
"""
...
OUT_DIR = ROOT / "output" / "data" / "detect_full"
OUT_DIR.mkdir(parents=True, exist_ok=True)
CROP_DIR = OUT_DIR / "crops"
CROP_DIR.mkdir(exist_ok=True)

PAGES = list(range(42))  # 0-41
...
def calc_metrics(blocks, ocr_time, detect_time):
    total = len(blocks)
    non_empty = sum(1 for b in blocks if b.get("ocr_text", ""))
    raw_chars = sum(len(b.get("ocr_text", "")) for b in blocks)
    short_boxes = sum(1 for b in blocks if len(b.get("ocr_text", "")) <= 3)
    punct_only = sum(1 for b in blocks if b.get("ocr_text", "") and all(c in "．。・〜~…—-「」『』、！？!?　 " for c in b.get("ocr_text", "")))
    return {
        "n_boxes": total,
        "non_empty_boxes": non_empty,
        "non_empty_rate": round(non_empty / total, 3) if total else 0.0,
        "raw_chars": raw_chars,
        "short_boxes_le3": short_boxes,
        "punct_only_boxes": punct_only,
        "avg_chars_per_box": round(raw_chars / total, 1) if total else 0.0,
...
def main():
    print(f"[detect-full] {len(PAGES)} pages, RT-DETR-v2 + Baberu OCR")
    det = RTDetrDetector(conf_threshold=0.3)
    det._load()
    print("[detect-full] model loaded.")
    ...
    for p in PAGES:
        ...
        metrics = calc_metrics(blocks, ocr_time, detect_time)
        metrics["image_size"] = [img_w, img_h]
        results.append({"page": p, **metrics})
        print(f"  page_{p:02d}: {metrics['n_boxes']:2d} boxes, {metrics['raw_chars']:4d} chars, "
              f"non_empty={metrics['non_empty_rate']:.0%}, "
              f"short<={metrics['short_boxes_le3']}, punct={metrics['punct_only_boxes']}, "
              f"det={metrics['detect_time_sec']:.2f}s, ocr={metrics['ocr_time_sec']:.2f}s", flush=True)
    ...
    agg = {
        ...
        "avg_nonempty_rate": round(sum(r["non_empty_rate"] for r in valid) / len(valid), 3),
        "total_short_boxes": sum(r["short_boxes_le3"] for r in valid),
        "total_punct_boxes": sum(r["punct_only_boxes"] for r in valid),
        "total_detect_time": round(sum(r["detect_time_sec"] for r in valid), 1),
        "total_ocr_time": round(sum(r["ocr_time_sec"] for r in valid), 1),
    ...
    print(f"\n{'='*60}")
    print(f"[detect-full] DONE in {elapsed:.1f}s")
    ...
    print(f"  Non-empty rate: {agg['avg_nonempty_rate']:.0%}")
    print(f"  Short boxes (<=3 chars): {agg['total_short_boxes']} ({agg['total_short_boxes']/max(1,agg['total_boxes'])*100:.1f}%)")
    print(f"  Punctuation-only boxes: {agg['total_punct_boxes']} ({agg['total_punct_boxes']/max(1,agg['total_boxes'])*100:.1f}%)")
    print(f"  Detect: {agg['total_detect_time']}s total, OCR: {agg['total_ocr_time']}s total")
    print(f"  -> {out}")
