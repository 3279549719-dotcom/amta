...
三件自动维护：
  ...
  2. refresh_now    把 recent.md 最新一条摘要写入 now.md（压缩恢复现场用）；
     若 now.md 为空则从 recent 补。
  3. prune_tmp      清空 .remember/tmp 与项目 output 下的一次性临时目录（.pytest_cache 等），
     防止 pytest/调试残留堆积。

原则：幂等（重复跑无害）、只动 .remember 与临时目录、绝不碰 docs/ 权威源、
所有写操作显式 UTF-8（L30，Windows 编码坑）。用法: python scripts/memory_gc.py [--dry-run] [--today YYYY-MM-DD]
"""
...
def archive_today(root: Path, today: datetime.date, dry: bool) -> list[str]:
    """未归档 today-*.md → 标 .done；超龄 .done → archive.md。返回动作日志。"""
    log: list[str] = []
    r = root / ".remember"
    if not r.exists():
        return log
    for f in sorted(r.glob("today-*.md")):
        ...
        if f.name.endswith(".done.md"):
            ...
            continue
        # 未归档 → 标 .done（追加到 recent 头部在 refresh_now 里做？不，这里直接归档到 recent）
        body = _read(f)
        rec = r / "recent.md"
        if dry:
...
def refresh_now(root: Path, dry: bool) -> list[str]:
    """now.md = 最近摘要（压缩恢复现场）。空则从 recent 补。"""
    log: list[str] = []
    r = root / ".remember"
    now = r / "now.md"
    rec = r / "recent.md"
...
def prune_tmp(root: Path, dry: bool) -> list[str]:
    """清空 .remember/tmp 与 output 下的一次性临时目录。返回动作日志。"""
    log: list[str] = []
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--today", default=None, help="YYYY-MM-DD（默认今天，测试可注入）")
