...
输出: output/data/guardrails_exp_v3/result_v3_conf02_tri-state.json
"""
...
RESULTS_JSON = ROOT / "output" / "data" / "detect_conf02" / "results.json"
OUT_DIR = ROOT / "output" / "data" / "guardrails_exp_v3"
...
# === 主流程 ===
def run_experiment():
    print(f"=== Guardrails v3 Experiment — 三态 VLM + conf=0.2 检测 ===")
    ...
    out_path = OUT_DIR / "result_v3_conf02_tri-state.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2)

    print(f"\n{'='*60}")
    print(f"=== v3 SUMMARY (三态 VLM) ===")
