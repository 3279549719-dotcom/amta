...
- **工具面**：`src/amta/` 已按领域归入子包（公开导入重铺为 `from amta.<pkg>.<mod> import`）——`common/`（paths·metrics·geometry·images·config·evalkit CER/EM·workstate·tickets·pipeline_log，跨阶段共享）+ `stores/`（artifacts 产物契约单一事实源 ADR-024 ·artifact_store dir-as-index ·artifact_cache 指纹缓存）+ `backends/`（koharu_client REST 适配·koharu_blocks scene节点→blocks纯整形·runner 单页执行器·chat_client OpenAI兼容HTTP深模块·ocr_engines 本地OCR·vlm_verify）+ `guards/`（guardrails 机械护栏·glossary Knowledge guardrail·canon_schema Input gate·suggestions 片假名提取追加合并·term_dict/replace·rule_filter 退役·vlm_filter·pre_scan）+ `inpaint/`（inpaint_station·inpaint_strategy·local_lama_inpainter·_lama_*，Stage 4，ADR-029/030/031）+ `typeset/`（typeset_engine·typeset_render·typeset_station·fonts，Stage 5-6，ADR-019/020/021）+ `translation/`（translate 编排·stage3_minimal（Stage3核心含 build_semantic_context 前页上下文，ADR-014/016/023）·translate_station）+ `stations/`（detect_station·ocr_station，Stage 1-2 深工位，脚本薄 CLI）+ `memory/`（agent 记忆：estate/tools/lint，ADR-025）+ `report/`（深接口 HTML 报告引擎，6阶段可插拔，统一入口 `scripts/gen_report.py --type {pipeline,final,stage4,inpaint_ab,ab}`，已在 main）。各包内模块清单见各自 `src/amta/<pkg>/__init__.py` 文档字符串。
