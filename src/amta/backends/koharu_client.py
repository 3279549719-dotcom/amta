...
class KoharuClient:
    ...
    def collect_blocks(nodes: dict[str, dict]) -> list[dict]:
        """节点→blocks 纯整形唯一归属 amta.backends.koharu_blocks（兼容别名，见 koharu_blocks.py）。"""
        from amta.backends.koharu_blocks import collect_blocks as _cb
        return _cb(nodes)

    @staticmethod
    def sort_by_reading_order(blocks: list[dict]) -> list[dict]:
        """日漫阅读序排序唯一归属 amta.backends.koharu_blocks（兼容别名，见 koharu_blocks.py）。"""
        from amta.backends.koharu_blocks import sort_by_reading_order as _sbo
        return _sbo(blocks)
