"""FFC (Fast Fourier Convolution) ResNet Generator — 从 BallonsTranslator 复制。

用于加载 lama-manga.safetensors 权重 (large arch, n_blocks=18)。
原始来源: reference/repos/BallonsTranslator/ballontranslator/modules/inpaint/ffc.py + lama.py
论文: Fast Fourier Convolution NeurIPS 2020
"""
...
class FourierUnit(nn.Module):
    def __init__(self, in_channels, out_channels, groups=1, spatial_scale_factor=None,
        ...
        self.conv_layer = nn.Conv2d(
            ...
            out_channels=out_channels * 2, kernel_size=1, stride=1, padding=0,
...
class FFCResNetGenerator(nn.Module):
    """LaMa Generator with FFC (Fast Fourier Convolution) blocks.
    ...
    def __init__(self, input_nc=4, output_nc=3, ngf=64, n_downsampling=3, n_blocks=18,
        ...
        out_ffc_kwargs = out_ffc_kwargs or {}
    ...
    def forward(self, img, mask):
        """img: (B,3,H,W) in [-1,1], mask: (B,1,H,W) in {0,1}.
        Returns: (B,3,H,W) — 注意 lama-manga 无输出激活, 值域不是 [-1,1].
        """
