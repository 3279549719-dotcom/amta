...
2. **判断链五路分流**：`观察 → 可复用?No 丢弃 / Yes → 会复发?No lesson(docs/lessons.md) / Yes → 五路：全局规则(CLAUDE.md) · 流程(.dsh/skills/) · 架构(docs/decisions/ADR-N) · 瞬时(commit body) · 机械(test/lint/hook)`。**test/lint/hook 是唯一真强制层**，rules/lesson 均为 prompt 级。
