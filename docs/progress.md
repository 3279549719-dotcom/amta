...
## 当前状态（2026-07，最后一笔：CLAUDE.md v2 重构）

- 工程脚手架已就位并推送（commit `854e170`、`119df6e`，origin/main 已同步）。
...
- **Benchmark A/B/C 已实现**（`src/benchmark.py` 两阶段：emit crops+manifest → ingest 标注算指标；synthetic 自测通过，compile PASS）。测试集预筛与首跑待做。
...
| 1 | Benchmark A/B/C（检测 recall / OCR CER / inpaint 评分） | 🔄 实现 benchmark.py 中 |
| 1b | 预筛 15 页测试集（describe_image，4 类覆盖） | ⏳ 未开始 |
...
1. 预筛 15 页测试集（`npm run prescreen -- D:\我的汉化\output\灵梦和妹红\process`，describe_image 初筛 4 类覆盖）。
2. 跑 Benchmark A（四 detector 同页对比），产出 output/benchmark_a.json。
3. 结果回填本节「当前状态」并推送。
